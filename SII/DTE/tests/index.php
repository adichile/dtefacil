...
try{
    ...
    $Documento->getEncabezado()->getIdDoc()->setTipoDTE("33");
    $Documento->getEncabezado()->getIdDoc()->setFolio("1");
    $Documento->getEncabezado()->getIdDoc()->setFchEmis("2019-04-22");

    $Documento->getEncabezado()->setEmisor();
    $Documento->getEncabezado()->getEmisor()->setRUTEmisor("76250838-9");
    $Documento->getEncabezado()->getEmisor()->setRznSoc(mb_substr("ASESORIA DISENO E INGENIERIA SPA", 0, 100));
    ...
    $Documento->getEncabezado()->getEmisor()->setGiroEmis("FACTURACION ELECTRONICA");

    /* Repetir por cada actividad econimica con un maximo de 4 */
    $Documento->getEncabezado()->getEmisor()->setActeco("720000");

    $Documento->getEncabezado()->setReceptor();
    $Documento->getEncabezado()->getReceptor()->setRUTRecep("66666666-6");
    $Documento->getEncabezado()->getReceptor()->setRznSocRecep("RAZON SOCIAL");
    $Documento->getEncabezado()->getReceptor()->setGiroRecep(mb_substr("GIRO", 0, 40));
    $Documento->getEncabezado()->getReceptor()->setCmnaRecep("COMUNA");
    $Documento->getEncabezado()->getReceptor()->setCiudadRecep("CIUDAD");
    $Documento->getEncabezado()->getReceptor()->setDirRecep(mb_substr("DIRECCION", 0, 70));

    $Documento->getEncabezado()->setTotales();
    $Documento->getEncabezado()->getTotales()->setMntNeto("10000");
    $Documento->getEncabezado()->getTotales()->setIVA("1190");
    $Documento->getEncabezado()->getTotales()->setMntTotal("11190");
    $Documento->getEncabezado()->getTotales()->setTasaIVA("19.00");
    /*
     * Para monto exento
     * $Documento->getEncabezado()->getTotales()->setMntExe("montoexento");
     *
     */
    ...
    $Documento->TED->DD->setIT1(mb_substr($Documento->getDetalle()[0]->getNmbItem(),0,40));
